...
- **M5 (task-06) выполнен, релиз 1.0.0.** Версия в pom и fallback App → 1.0.0; tar
  пересобраны (jar внутри печатает «genpass 1.0.0» — подтверждено запуском из tar).  Результаты раздела 10: (1) `mvn clean verify` SUCCESS — 59 тестов (42 unit + 17 integration);
  ...
  ПО ЗАМЕЧАНИЮ ПОЛЬЗОВАТЕЛЯ «не везде тёмная тема»: UI переведён на ПОЛНУЮ тёмную тему
  (страница #12151C, панели #1A1F2A, инсет результата #0F1219, акцент #4C9A82 — контрасты
  ≥4.5:1 сохранены; light-вариант удалён). Проверено CDP computed-стилями (все токены
  точь-в-точь), видимость панелей flex,none,none, генерация 16 симв., JS-ошибок 0;
  tar пересобраны с тёмной темой (CSS #12151C в отдаваемом style.css подтверждён).
  ОСТАЁТСЯ ЗА ПОЛЬЗОВАТЕЛЕМ: деплой на NAS (критерий 9).
  ✅ ПРИЁМКА ПОДТВЕРЖДЕНА ПОЛЬЗОВАТЕЛЕМ (релиз 1.0.0, тег v1.0.0).
